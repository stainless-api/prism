...
const buildSchemaFilter = <S extends RequiredSchemaSubset>(
  ...
  function filterPropertiesFromObjectSingle(schema: S): Option<S> {
    return pipe(
      ...
      O.alt(() =>
        pipe(
          // the schema is an tuple-typed array with additionalItems schema defined
      ...
      O.map(filteredProperties => {
        if (schema.items) {
          ...
          if (Array.isArray(schema.items) && typeof schema.additionalItems === 'object') {
            return {
              // the array is tuple-typed with additionalItems schema object specified
              ...schema,
              additionalItems: {
        ...
        return {
          // the schema is a non-array object
          ...schema,
          properties: filteredProperties,
        };
      }),
    ...
    return pipe(
      ...
      O.chain(itemSchemas =>
        pipe(
          ...
          O.map(zippedSchemas =>
            zippedSchemas.map(([itemSchema, originalItemSchema]) =>
              filterRequiredHelper(itemSchema, originalItemSchema)
            )
          )
        )
      ),
